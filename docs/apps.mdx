...
by the space-server. Communication happens through two shared types on the Y.Doc:
...
Apps are built using the **space-app** skill, defined in the SKILL.md file. When a bot has this skill, it automatically:

1. Connects to the space-server via WebSocket with token authentication
...
The space-server is a lightweight WebSocket server that hosts the shared Y.Doc. It:
...
- Connects to the space-server with exponential backoff reconnection
...
The space-server authenticates WebSocket connections via a token query parameter:

```
ws://space-server:1234?token=YOUR_TOKEN
...
const doc = new Y.Doc();
const provider = new WebsocketProvider('ws://space-server:1234', 'doc', doc, {
